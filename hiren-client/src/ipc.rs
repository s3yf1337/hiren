//! IPC client for hiren-daemon (unchanged protocol)
...
pub fn search_sync(query: &str) -> Result<Vec<AppEntry>> {
    ...
    stream.set_read_timeout(Some(Duration::from_secs(3))).context("Failed to set read timeout")?;
    stream.set_write_timeout(Some(Duration::from_secs(3))).context("Failed to set write timeout")?;
    ...
    if body_len > hiren_shared::MAX_FRAME_SIZE { anyhow::bail!("Response too large: {body_len} bytes"); }
